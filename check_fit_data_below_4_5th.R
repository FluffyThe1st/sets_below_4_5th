...
check_fit_below_4_5th <- function(cond, model)
{
  ...
  assign(paste0(model, "_sets_of_cond", cond, "_below_4_5th"), below, envir = .GlobalEnv)
  remove(test, mdl_fit, n_T, NB, n_B, below, j)
  save.image(file = paste0(model, "_sets_of_cond", cond, "below_4_5th.Rdata"))
}
for (cond in 1:12){
  check_fit_below_4_5th(cond, "std")
  remove(list = ls()[2])
}